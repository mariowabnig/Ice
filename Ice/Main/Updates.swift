...
final class UpdatesManager: NSObject, ObservableObject {
    ...
    private(set) lazy var updaterController = SPUStandardUpdaterController(
        startingUpdater: !CustomBuild.isEnabled,
    ...
    var automaticallyChecksForUpdates: Bool {
        get {
            guard !CustomBuild.isEnabled else { return false }
            return updater.automaticallyChecksForUpdates
        }
        set {
            guard !CustomBuild.isEnabled else { return }
    ...
    @objc func checkForUpdates() {
        if CustomBuild.isEnabled {
            NSWorkspace.shared.open(CustomBuild.updatesURL)
            return
        }
        #if DEBUG

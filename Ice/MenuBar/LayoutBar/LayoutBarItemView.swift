        ...
        if let appState {
            appState.imageCache.$images
                .sink { [weak self] images in
                    guard let self, let cachedImage = images[item.windowID] else {
                        return
                    }
                    if item.isAuxiliaryStatusItem,
                       let trimmed = cachedImage.cgImage.trimmingTransparency(around: [.minXEdge, .maxXEdge]) {
                        self.cachedImage = .init(cgImage: trimmed, scale: cachedImage.scale)
                    } else {

...
final class ControlItem {
    ...
    private func configureCancellables() {
        ...
        statusItem.publisher(for: \.isVisible)
            ...
            .sink { [weak self] isVisible in
                ...
                if #available(macOS 27.0, *) {
                    // Modern sections are app assignments, not physical divider
                    // status items. Keep section hotkeys available even when a
                    // legacy divider is hidden or removed.
                    hotkey.enable()
                } else if isVisible {
                    hotkey.enable()
                } else {
        ...
        if let appState {
            ...
            if identifier == .visible {
                ...
                if #available(macOS 27.0, *) {
                    appState.modernMenuBarManager.$revealed
                        .removeDuplicates()
                        .receive(on: DispatchQueue.main)
                        .sink { [weak self] _ in
                            self?.updateStatusItem()
    ...
    private func updateAuxiliaryStatusItemReservationFrames(from items: [MenuBarItem], for state: HidingState? = nil) {
        ...
        _ = auxiliaryStatusItemReservationLength(for: .showSection)
    }
    ...
    private func auxiliaryStatusItemReservationLength(for state: HidingState? = nil) -> CGFloat {
        ...
        let dividerFrame = frame ?? window?.frame
    ...
    private func auxiliaryStatusItemFrames(from items: [MenuBarItem], includeCachedFrames: Bool = true) -> [CGRect] {
        ...
        let liveWindowFrames = WindowInfo
            .createWindows(option: .onScreen)
            .filter(isAuxiliaryStatusItemWindowFallback)
            .map(\.bounds)
        return (cachedFrames + itemFrames + liveWindowFrames).reduce(into: []) { frames, frame in
    ...
    private func isAuxiliaryStatusItemWindowFallback(_ window: WindowInfo) -> Bool {
        guard
            ...
            window.isOnScreen,
            window.bounds.height <= auxiliaryStatusItemMaxHeight(for: window)
        else {
            return false
        }
    ...
    private func auxiliaryStatusItemMaxHeight(for window: WindowInfo) -> CGFloat {
        let screen = NSScreen.screens.first { CGDisplayBounds($0.displayID).intersects(window.bounds) }
    ...
    private func updateStatusItem() {
        ...
        case .visible:
            ...
            let iconState: HidingState
            if #available(macOS 27.0, *) {
                iconState = appState.modernMenuBarManager.revealed.isEmpty ? .hideSection : .showSection
            } else {
                iconState = state
            }

            // We can usually just create the image directly from the icon.
            var image = switch iconState {
        ...
        case .hidden, .alwaysHidden:
            switch state {
            case .showSection:
                let reservationLength = identifier == .hidden ? auxiliaryStatusItemReservationLength(for: state) : 0
                let shouldReserveAuxiliaryItemSpace = reservationLength > 0

                switch appState.settings.advanced.sectionDividerStyle {
                case .noDivider:
                    updateStatusItemVisibility(shouldReserveAuxiliaryItemSpace, length: Lengths.standard + reservationLength)
                ...
                case .chevron:
                    updateStatusItemVisibility(true, length: Lengths.standard + reservationLength)
                    button.appearsDisabled = false
    ...
    private func updateStatusItemVisibility(_ isVisible: Bool, length: CGFloat? = nil) {
        if #available(macOS 27.0, *), isSectionDivider {
            constraint?.isActive = false
            statusItem.length = 0
            if let window {
        ...
        if isVisible {
            ...
            statusItem.length = length ?? identifier.length(for: state)
        } else {
        ...
        case .leftMouseDown:
            ...
            Task {
                ...
                if
                    ...
                    appState?.settings.advanced.canToggleAlwaysHiddenSection == true,
    ...
    private func createMenu(with appState: AppState) -> NSMenu {
        ...
        for name: MenuBarSection.Name in [.hidden, .alwaysHidden] {
            guard
                name != .alwaysHidden || appState.settings.advanced.canToggleAlwaysHiddenSection,

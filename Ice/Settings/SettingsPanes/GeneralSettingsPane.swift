...
struct GeneralSettingsPane: View {
    ...
    private var iceBarOptions: some View {
        if #available(macOS 27.0, *) {
            Text("Hidden items open in the system menu bar on macOS 27. Use Menu Bar Layout to arrange them.")
                .foregroundStyle(.secondary)
        } else {
            useIceBar
    ...
    @ViewBuilder
    private var modernVisibilityStatus: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("macOS 27 hides saved sections after Menu Bar Layout closes or another settings page is selected.")
                .foregroundStyle(.secondary)
            ModernVisibilityStatusView(manager: appState.modernMenuBarManager)
        }
    }
    ...
    private var rehideStrategyPicker: some View {
        VStack {
            ...
            .annotation {
                ...
                case .timed:
                    Text("Menu bar items rehide after the pointer has been away from the menu bar for this long.")
                case .focusedApp:

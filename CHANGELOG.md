...
- Native hidden-section reveal now hides application menus first when that setting is enabled and briefly lets the layout settle before WindowServer starts dropping status items, so the expand control can make room on crowded menu bars.

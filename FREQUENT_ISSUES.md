...
When the hidden section is shown in the native menu bar, Ice also keeps enough invisible divider width reserved for these auxiliary status windows. The reservation uses the auxiliary window frames captured while the hidden section is hidden and merges in live on-screen auxiliary frames when the cached visible-section list is stale, so newly shown native items are added to the left without pushing the auxiliary window around. If the modeled menu bar item list temporarily misses an app-owned overlay, Ice falls back to directly scanning WindowServer for status-level windows owned by non-system apps. If the hidden-section divider frame lands inside an auxiliary overlay, Ice reserves across the full auxiliary frame instead of only reserving up to the divider edge. If the divider frame is temporarily unavailable during the first reveal layout pass, Ice still reserves the auxiliary overlay's full width plus the clearance the settled divider-frame calculation normally needs, and keeps the last valid reservation while macOS is reshuffling windows. The spacer is primed while the section is hidden and is allowed to grow, but not shrink, until the section hides again. This prevents hidden native menu bar items from sliding under an auxiliary overlay window that macOS does not manage as a normal status item.

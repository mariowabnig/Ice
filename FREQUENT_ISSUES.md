...
The `build-and-install.sh` helper preserves existing TCC permissions. It no longer calls `tccutil reset` for Accessibility or Screen Recording during install. It creates or reuses a self-signed `Ice Local Development` code-signing identity in the login keychain when possible, then signs the embedded Sparkle framework and Ice app with that stable identity before launching the new build.

The first install after switching from ad-hoc signing to `Ice Local Development` may require one new Accessibility approval. Later local rebuilds should keep that grant as long as `/Applications/Ice.app`, the bundle identifier, and the local certificate remain stable. If identity creation fails and the script falls back to ad-hoc signing, remove Ice from `System Settings` > `Privacy & Security` > `Accessibility`, add `/Applications/Ice.app` again, and relaunch Ice. Stable Developer ID signing is still the best long-term distribution identity outside local development.

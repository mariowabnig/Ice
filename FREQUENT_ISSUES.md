...
Before a native hidden-section reveal, Ice also hides the application menus first when the user has enabled that setting, then waits briefly for the native menu bar layout to settle. Doing this before the status item lengths change prevents macOS from dropping items during the reveal and then leaving the expanded layout no better than the hidden layout.

Some auxiliary status windows are shorter than the system menu bar and are pinned by macOS to the menu bar's top edge. Ice cannot move those third-party windows directly, so it marks them as non-movable in the Menu Bar Layout view. It also places a pass-through visual cover over the item while the menu bar is visible, redraws the menu bar background, and paints the captured auxiliary item centered within the full menu bar height. The cover ignores mouse events, so clicks still reach the owning app's real status window.
